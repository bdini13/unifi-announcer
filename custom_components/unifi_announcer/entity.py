...
def migrate_generated_names(entry, coordinator) -> None:
    ...
    service = device_registry.async_get_device_by_identifier(
        (DOMAIN, entry.data["url"]), entry.entry_id
    )
    ...
    for target, chime_id, is_group in configured_targets(coordinator):
        ...
        device = device_registry.async_get_device_by_identifier(
            (DOMAIN, str(chime_id)), entry.entry_id
        )
...
class UniFiAnnouncerEntity(CoordinatorEntity):
    ...
    def device_info(self) -> DeviceInfo:
        ...
        if self.target and self.chime_id and not self.is_group:
            ...
            service = dr.async_get(
                self.coordinator.hass
            ).async_get_device_by_identifier(service_identifier, self.entry.entry_id)
            if service is not None:
                device_info["via_device_id"] = service.id
            return device_info
        return DeviceInfo(
            identifiers={service_identifier},
